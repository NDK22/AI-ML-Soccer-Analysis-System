...
class Tracker:
    ...
    def draw_ellipse(self, frame, bbox, color, track_id=None):
        ...
        if track_id is not None:
            cv2.rectangle(frame,(int(x1_react),int(y1_react)),(int(x2_react),int(y2_react)), color, cv2.FILLED)
            x1_text = x1_react +12
            if track_id > 99:
                x1_text -=10
            cv2.putText(frame, f"{track_id}", (int(x1_text), int(y1_react+15)), cv2.FONT_HERSHEY_SIMPLEX,0.6,(0,0,0),2)
        return frame
    ...
    def draw_triangle(self,frame, bbox,color):
        ...
        cv2.drawContours(frame,[triangle_points],0,(0,0,0))
        return frame
