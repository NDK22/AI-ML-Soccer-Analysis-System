...
class CameraMovementEstimator():
    ...
    def get_camera_movement(self,frames,read_from_stub=False, stub_path=None):
        ...
        for frame_num in range(1,len(frames)):
            ...
            new_features, _, _ = cv2.calcOpticalFlowPyrLK(old_gray,frame_grey,old_features, None, **self.lk_params)
    ...
    def draw_camera_movement(self,frames, camera_movement_per_frame):
        ...
        for frame_num, frame in enumerate(frames):
            frame= frame.copy()
            overlay = frame.copy()
            cv2.rectangle(overlay,(0,0),(500,75),(255,255,255),-1)
            alpha =0.6
            cv2.addWeighted(overlay,alpha,frame,1-alpha,0,frame)
            x_movement, y_movement = camera_movement_per_frame[frame_num]
            frame = cv2.putText(frame,f"Camera Movement X: {x_movement:.2f}",(10,30), cv2.FONT_HERSHEY_TRIPLEX,0.9,(0,0,0),1)
            frame = cv2.putText(frame,f"Camera Movement Y: {y_movement:.2f}",(10,60), cv2.FONT_HERSHEY_TRIPLEX,0.9,(0,0,0),1)
